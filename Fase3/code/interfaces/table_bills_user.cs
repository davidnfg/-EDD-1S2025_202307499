...
namespace code.interfaces
{
    public class UserWindowTableInvoices : Window
    {
        ...
        public UserWindowTableInvoices(List<Factura> listaFacturas) : base("Facturas")
        {
            ...
            treeViewFacturas.AppendColumn("Total", new CellRendererText(), "text", 4);

            // Crear el ListStore para almacenar los datos
            listStoreFacturas = new ListStore(typeof(int), typeof(int), typeof(string), typeof(string) ,typeof(double));
        ...
        private void MostrarFacturas(List<Factura> facturas)
        {
            ...
            {
                listStoreFacturas.AppendValues(factura.ID, factura.ID_Servicio, factura.Fecha , factura.MetodoPago,factura.Total);
            }
        }
    }
}