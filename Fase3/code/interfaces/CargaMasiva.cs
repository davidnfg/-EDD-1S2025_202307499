...
public class CargaMasivaGTK : Window
{
    ...
    public CargaMasivaGTK() : base("Carga Masiva desde JSON")
    {
        ...
        comboTipo.AppendText("Servicios");
        comboTipo.Active = 0;
            ...
            switch (tipo)
            {
                ...
                case "Usuarios":
                    foreach (var item in jsonArray)
                    {
                        ...
                        code.data.Variables.listaUsuarios.AddBlock(id, nombres, apellidos, correo, edad, contrasenia);
                ...
                case "Servicios":
                    foreach (var item in jsonArray)
                    {
                        try
                        {
                            ...
                            if (repuesto == null)
                            {
                                Console.WriteLine($"Repuesto con ID {idRepuesto} no encontrado. Servicio no agregado.");
                                continue;
                            }

                            if (vehiculo == null)
                            {
                                Console.WriteLine($"Vehículo con ID {idVehiculo} no encontrado. Servicio no agregado.");
                                continue;
                            }
                            ...
                            code.data.Variables.arbolFacturas.Insert(idFactura, id, total, fecha, metodoPago);
                            Console.WriteLine($"Factura generada: ID={idFactura}, Id_Servicio={id}, Total={total}, Método Pago={metodoPago}");
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"Error al procesar un servicio: {ex.Message}");
                        }
                    }
                    statusLabel.Text = $"Cargados {jsonArray.Count} servicios y generadas sus facturas.";
                    break;
